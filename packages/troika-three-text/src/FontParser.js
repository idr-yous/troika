 ...
 * Optionally uses HarfBuzz WASM for advanced text shaping when configured.
 */

import typrFactory from "../libs/typr.factory.js";
import woff2otfFactory from "../libs/woff2otf.factory.js";
import harfbuzzFactory from "../libs/harfbuzz.factory.js";
...
function parserFactory(Typr, woff2otf, getHb) {
          ...
          for (let j = 0; j < lookup.tabs.length; j++) {
            ...
            // Mark to base
              ...
              if (markArrIndex !== -1) {
                ...
                const baseArrIndex =
                  baseGlyphIndex === -1 ? -1 : Typr._lctf.coverageIndex(tab.baseCoverage, glyphIds[baseGlyphIndex]);
                if (baseArrIndex !== -1) {
                  const markRecord = tab.markArray[markArrIndex];
                  const baseAnchor = tab.baseArray[baseArrIndex][markRecord.markClass];
                  positions[glyphIndex * 3] =
                    baseAnchor.x - markRecord.x + positions[baseGlyphIndex * 3] - positions[baseGlyphIndex * 3 + 2];
                  ...
                  if (getGlyphClass(font, prevGlyphId) === 3) {
                    ...
                    if (mark2ArrIndex !== -1) {
                      ...
                      positions[glyphIndex * 3] =
                        mark2Anchor.x -
                        mark1Record.x +
                        positions[prevGlyphIndex * 3] -
                        positions[prevGlyphIndex * 3 + 2];
  ...
  /**
   * forEachGlyph implementation using HarfBuzz for text shaping.
  ...
  /**
   * Original forEachGlyph implementation using Typr for text shaping.
   * Used as fallback when HarfBuzz is not configured.
   */
  function forEachGlyphTypr(typrFont, fontObj, text, fontSize, letterSpacing, callback, getGlyphObj) {
    let penX = 0;
    const fontScale = (1 / fontObj.unitsPerEm) * fontSize;
  ...
  return function parse(buffer) {
    ...
    return wrapFontObj(Typr.parse(buffer)[0], buffer);
  };
}

/**
 * Create a font parser worker module, optionally configured with HarfBuzz WASM
 * for advanced text shaping.
 *
 * @param {object} [options]
 * @param {string} [options.harfbuzzWasmUrl] - URL to hb.wasm file. When provided,
 *   HarfBuzz will be used for text shaping instead of the built-in Typr shaping.
 *   This enables correct rendering of complex scripts (Arabic, Devanagari, etc.)
 * @returns {object} A defineWorkerModule handle for the font parser
 */
export function createFontParserModule({ harfbuzzWasmUrl } = {}) {
  if (harfbuzzWasmUrl) {
    ...
    return defineWorkerModule({
      ...
      init(typrFactory, woff2otfFactory, parserFactory, harfbuzzFactory, wasmUrl) {
  ...
  // Default: no HarfBuzz, use Typr-only shaping
  return defineWorkerModule({
    name: "Typr Font Parser",
...
// Default export: backward-compatible static module (no HarfBuzz)
const defaultModule = /*#__PURE__*/ createFontParserModule();
