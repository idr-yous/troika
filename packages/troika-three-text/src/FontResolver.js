...
export function createFontResolver(fontParser, unicodeFontResolverClient) {
  ...
  return function (
    text,
    callback,
    { lang, fonts: userFonts = [], style = "normal", weight = "normal", unicodeFontsURL } = {},
  ) {
...
/**
 * Create a font resolver worker module with the given font parser module.
 * @param {object} [fontParserModule] - A defineWorkerModule handle for the font parser.
 *   Defaults to the built-in Typr-only parser.
 * @returns {object} A defineWorkerModule handle for the font resolver
 */
export function createFontResolverModule(fontParserModule) {
  return defineWorkerModule({
    name: "FontResolver",
    dependencies: [createFontResolver, fontParserModule || fontParser, unicodeFontResolverClientFactory],
...
// Default export: backward-compatible static module (uses default Typr-only parser)
export const fontResolverWorkerModule = /*#__PURE__*/ createFontResolverModule();
